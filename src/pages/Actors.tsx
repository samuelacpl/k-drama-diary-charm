  ...
  const actors = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "watched":
        return list.sort(
          (a, b) =>
            b.dramas.length - a.dramas.length || a.name.localeCompare(b.name),
        );
      case "alpha":
      default:
      ...
      <div className="container max-w-3xl py-6 space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            {isLoading
              ? "Loading actors..."
              : `${actors.length} actors across your dramas`}
          </p>
        </div>
            ...
            {actors.length === 0 ? (
              <div className="text-center py-16 space-y-3">
                ...
                <p className="text-muted-foreground">
                  No actors yet! Add dramas with TMDb search to see actors here.
                </p>
              </div>
            ) : (
                ...
                {actors.map((actor) => {
                  ...
                  return (
                    <Link
                      key={actor.id}
                      to={`/actor/${actor.id}`}
                      className="glass-card rounded-2xl p-4 text-center space-y-2 animate-fade-in hover:shadow-md transition-all hover:-translate-y-0.5 cursor-pointer"
                    >
                      ...
                      <p className="text-xs text-muted-foreground">
                        {actor.dramas.length} drama
                        {actor.dramas.length > 1 ? "s" : ""}
                      </p>
